package com.neptum.integrationtests.controller.withyml.mapper
...
class YMLMapper: ObjectMapper {
    ...
    override fun deserialize(context: ObjectMapperDeserializationContext): Any? {
        try{
            ...
            logger.info("Trying deserialize object type $type")
    ...
    override fun serialize(context: ObjectMapperSerializationContext): Any? {
        try{
            logger.info("trying seriaize object ${context.objectToSerialize}")
            return objectMapper.writeValueAsString(context.objectToSerialize)
        } catch (e: JsonProcessingException) {
            logger.severe("Error serializng object")
            e.printStackTrace()
        }
        return null
    }
